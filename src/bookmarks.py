...
def handle_bookmarks():
    ...
    if request.method == 'POST':
        ...
        return jsonify({'data': data, "meta": meta}),status_codes.HTTP_200_OK
...
def delete_bookmark(id):
    ...
    if not bookmark:
        return jsonify({'message': 'Item not found'}),status_codes.HTTP_404_NOT_FOUND

    db.session.delete(bookmark)
    db.session.commit()

    return jsonify({}),status_codes.HTTP_204_NO_CONTENT
